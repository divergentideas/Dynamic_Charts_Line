...
class elem():
    ...
    def __init__(self, name=None, canvas=None, y=0, unit=None, font_color=(0,0,0), colors=None, font=None, parent=None):
        ...
        # load image
        try:
            self.img = cv.load_image(os.path.join("assets_for_line_chart", self.name.replace("*", "") + ".png"), 4*int(self.parent.font_size), 4*int(self.parent.font_size), self.parent.root, name)
        except:
