...
# them valuables: muon sort legend thi chuyen no thanh false
sorting=True
...
class canvas():
    ...
    def __init__(self, width=None,line_width=20,point_radius=20, height=None, bg=(255, 255, 255), colors={}):
        """
    ...
    def add_sub_plot(self, sub_plot):
        ...
        :type sub_plot: thebeautyofdata.Canvas.sub_plot
        """
    ...
    def play(self, df=None, fps=30, save_image=False, location=None, x1=0, y1=0, x2=1500, y2=1080):
        """
        ...
        for i, date_time in enumerate(df.index):
            ...
            if save_image==True:
                # ImageGrab.grab(bbox=((1920-cut_x_pos)/2, 0, (1920-cut_x_pos)/2+cut_x_pos, cut_y_pos)).save(location+"..\\..\\export_all\image"+str(i)+".png", "PNG")
                ...
                # Grab the region and save it as an image
                ImageGrab.grab(bbox=(x1, y1, x2, y2)).save(location + str(i) + ".png", "PNG")

                print(i)


    def add_title(self, text,font_size=20, color=(0, 0, 0)):
        """
        ...
        """
        # title_font = font.Font(family=text_font, size=int(self.height/40/ SCALEFACTOR), weight="bold")
        title_font = font.Font(family=text_font1, size=int(font_size), weight="bold")
        self.canvas.create_text(self.width/2, self.height/20-10, font=title_font, text=text, fill=_from_rgb(color))

    def add_sub_title(self, text,font_size=20, color=(0, 0, 0)):
        """
        ...
        """
        title_font = font.Font(family=text_font1, size=int(font_size), weight="bold")
        self.canvas.create_text(self.width/2, self.height/11-10, font=title_font, text=text, fill=_from_rgb(color))

    def add_time(self, df, time_indicator="year", color=(0, 0, 0),font_size=20,x=800,y=80):
        """
        ...
        sub_plot = Date.date(canvas=self.canvas, start_time=list(df.index)[0], width=0, height=font_size*3,
                                       x_pos=x, y_pos=y, time_indicator=time_indicator, anchor='e',
                                       font_color=color)
        self.add_sub_plot(sub_plot)

    def add_logo(self, logo,width_ratio=15,height_ratio=15, x_pos=WIDTH*0.29,y_pos=HEIGHT*0.55 ):
        """
        ...
        :type str
        """
        from thebeautyofdata_vertical_screen import StaticImage
        img = StaticImage.static_image(canvas=self.canvas, width=int(WIDTH/width_ratio), height=int(WIDTH/height_ratio), x_pos=x_pos,
                                           y_pos=y_pos,
                                           file=logo, root=self.tk, anchor="ne",on_top=True)
...
class sub_plot():
    ...
    """
    def __init__(self, canvas=None, width=None,makersize=10,linesize=20, height=None, x_pos=None, y_pos=None, start_time=None, text=None, df=None, multi_color_df=None, anchor="c", sort=sorting, colors={}, root=None, display_percentages=True, display_label=True, title=None, invert=False, origin="s", display_value=True, font_color=(0,0,0), back_ground_color=(255,255,255), events={}, time_indicator="year", number_of_bars=None, unit="", x_ticks = 4, y_ticks = 4, log_scale=False, only_show_latest_event=True, allow_decrease=True, format="Europe", draw_points=True, area=True, color_bar_color=[[100, 100, 100], [255, 0, 0]],line_width=1,point_radius=8, radius_limit_to_decrease_line_width=7,
                 final_radius=7, remove_point=1, size_of_legend=13,min_slice = 0.001,
                 min_slice_image = 0.055,
                 min_slice_percentage_display = 0.01,
                 decimal_places = 1,
                 min_value_at_start=10, chose_max_of_y_axis=168, max_of_y_axis=57,
                 move_chart_along_x=0,move_chart_along_y=0,speed_of_decrease_radius=0.005, **kwargs):
        """
        ...
        self.radius_limit_to_decrease_line_width = radius_limit_to_decrease_line_width
        self.final_radius = final_radius
        self.remove_point = remove_point
        self.size_of_legend = size_of_legend
        self.min_value_at_start = min_value_at_start
        self.chose_max_of_y_axis = chose_max_of_y_axis
        self.max_of_y_axis = max_of_y_axis
        self.move_chart_along_x = move_chart_along_x
        self.move_chart_along_y = move_chart_along_y
        self.speed_of_decrease_radius= speed_of_decrease_radius
        if width == None:
        ...
        if isinstance(canvas, tkinter.Canvas):
            self.canvas = canvas
            self.the_beauty_of_data_canvas = None
        elif isinstance(canvas, thebeautyofdata.Canvas.canvas):
            self.canvas = canvas.canvas
            self.the_beauty_of_data_canvas = canvas
        else:
            raise "Please set the canvas to a tkinter.Canvas or thebeautyofdata.Canvas"
        self.colors = colors
        self.root = root
        self.invert = invert
        self.origin = origin
