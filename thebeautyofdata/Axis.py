...
class axis():
    ...
    def draw(self, min=0, max=0):
        self.min = min
        self.max = max
        self.canvas.create_line(self.x, self.y, self.x+self.length-55, self.y, fill=cv._from_rgb(self.color), width=self.line_tickness)
    ...
    def update(self, min=0, max=0):
        ...
        if not self.is_date:
            ...
            self.min_date = (min - datetime.datetime(1700,1,1)).days
            self.max_date = (max - datetime.datetime(1700,1,1)).days
    ...
    def update(self, value=0, draw=True, l=0):
        ...
        if draw:
            if self.axis.is_date:
                t = datetime.datetime(1700,1,1) + datetime.timedelta(days=value)
                # label = t.strftime("%b/%Y")  # %b gives you the abbreviated month name, %Y the full year.
